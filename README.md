...
| Accounting | `GET/POST /accounting/accounts`, `GET/PATCH/DELETE /accounting/accounts/:id`; `GET/POST /accounting/journal-entries`, `POST /accounting/journal-entries/:id/approve`, `POST /accounting/journal-entries/:id/reverse`; `GET /accounting/trial-balance`, `GET /accounting/profit-loss`, `GET /accounting/balance-sheet`, `GET /accounting/general-ledger` (writes: ADMIN/MANAGER/ACCOUNTANT) — seeded chart of accounts per tenant, double-entry journal with DB-enforced balance (trigger), manual entries require approval, reversal bookkeeping, auto-posting of sales/purchases (confirm, payment, void) with full source traceability, financial reports |
